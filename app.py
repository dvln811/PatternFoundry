...
def check_auth():
    # Public paths
    public = ['/login', '/register', '/static', '/api/', '/robots.txt', '/sitemap.xml']
    path = request.path
...
def sim_session():
    ...
    # Generate 5-min history with proper session structure (off-hours dampened)
    # _build_session_timestamps runs continuously from 06:00 (skips weekends only)
    # 24hrs / 5min = 288 bars per day
    ...
    # Use structured candles directly as history (already has session-aware dampening)
    hist_1min = structured  # list of dicts with time, open, high, low, close, volume, session
